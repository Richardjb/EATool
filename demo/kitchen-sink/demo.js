...
env.editSnippets = function() {
    ...
    var enteredText = editor2.getValue();
    //split text into groups of lines
    var splt = enteredText.split("\n");
    ...
    $(".entry").each(function (){
        ...
        ttySearchVals.push($(this).find("input").val());
    });

    //alert($(".entry").find("input").val());


    var strContainer = "";
    for (var i = 0; i < splt.length; i++){
        for(var j = 0; j < ttySearchVals.length; j++){
            if (~splt[i].indexOf("[" + ttySearchVals[j] + "]")){
                strContainer += "\n" + splt[i];
            }
        }
        // console.log(splt[i] + " The line is: " + i);
    }
    console.log(editor.getValue());

    editor.setValue(strContainer);
